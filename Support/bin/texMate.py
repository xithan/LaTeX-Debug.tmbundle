...
#        else:
        ...
        texCommand = engine + " " + constructEngineOptions(tsDirs,tmPrefs)
        texStatus,isFatal,numErrs,numWarns = run_latex(texCommand,fileName,verbose)  # Hack: Eigentlich True = verbose, aber es funktionierte nicht über die Einstellugen im Latex-Bundle
        if engine == 'latex':
...
#
    ...
    if texStatus != 0 or numWarns > 0 or numErrs > 0:
        ...
        # Link-Version
        #print '<script> var i = 0; function nextError() { document.location = "#error"+i; if (link=document.getElementById("errorlink"+i)) document.getElementById("nexterror").href = link.getAttribute("href"); else (elem=document.getElementById("fastfinder")).parentNode.removeChild(elem); i = i+1;  }</script>'
        #print '<div id="fastfinder"><a id="nexterror" onClick="nextError();" href=""> >> </a></div>'
        print "<p class='info'>Found <a name=\"errorsummary\" href=\"#error0\">" + str(numErrs) + "</a> errors, and " + str(numWarns) + " warnings in " + str(numRuns) + " runs</p>"
        if texStatus:
            if texStatus > 0:
