...
# Changes by Philipp Kuehl, 2012, marked  with "be silent" and "#+"/"#-"
...
sub init_environment {
	...
	#+ be silent
	print '<script>function clear_page(cssid){if (elem=document.getElementById(cssid)){(clear=document.getElementById("clear")).parentNode.removeChild(clear);elem.parentNode.removeChild(elem);}}</script>';
	print '<div id="done"></div>';
}
...
sub main_loop {
	...
	while(1) {
		if (document_has_changed()) {
			...
			clear_html_output("done");
			print "<p class=\"info\">Processing...</p></div>";
			reload();
			compile() and view();
...
sub reload {
	...
	while (<$f>) {
		...
		# 			$body = $2;
		# 		}
		if (/(.*\\input\s*\{(?:currentheader)\})(.*)/) {
			$new_preamble .= $1;
			$body = $2;
		}
		elsif ((/(.*)(\\begin\s*\{(?:document)\}.*)/)) {
			$new_preamble .= $1;
			$body = $2;
		}
		elsif (defined $body) {
		...
		# 	elsif (/(.*\\fi)(.*)/) {
		# 		fail ("testund",
		# 					"I couldn't find the command \\begin{document} in your file, but it's there");
		# 		
		# 		$new_preamble .= $_;
		# 		$block -= 1;
		# 		if ($block == 0) {
	...
	if (!defined($preamble) or $new_preamble ne $preamble) {
		...
		clear_html_output("preamble");  #+ be silent
		regenerate_format($new_preamble);
	}
	clear_html_output("texbody"); #+ be silent
	save_body($body);
...
sub regenerate_format {
	...
	sub {
		#+ be silent
		parse_log_file("preamble","$dotname.ini");  #+
...
sub compile {
	...
	sub {
		if ($? ==1 || $? == 2) {
			...
			# be silent
			#- offer_to_show_log();
			clear_html_output("processing");
			#print '<div id="done"><p class="info">Error?.</p></div>';
			parse_log_file("texbody","$dotname.tex"); #+
			$error = 1;
		}
		else {
			clear_html_output("processing");
			#print '<div id="done"><p class="info">Fail?.</p></div>';
...
#		print '';
		clear_html_output("processing");
		print '<div id="done"><p class="info">Done.</p></div>';
	}
...
sub parse_log_file { #+
	my ($cssid, $texfilename) = @_;
	my $errors;
	my $ls_cmd = "python \"$ENV{TM_BUNDLE_SUPPORT}/bin/texparser.py\" $wd/$dotname.log $texfilename";
	print "<div id=\"$cssid\"><p class=\"info\">Error processing $cssid.</p>";
	...
	#$ausgabe = `$ls_cmd`;
	#
	#$errors =~ s/Found (.+?) errors./$1/;
	#print $errors;
	#print '--></div>';
	#print "<a>War das der Rueckgabewert</a>";
	
}

sub clear_html_output { #+
	my ($cssid) = @_;
	print  "<script id=\"clear\">clear_page(\"$cssid\");</script>";
}
...
sub start_pdf_viewer {
	#print "<!--";
	#my $capout = `open -a $pdf_viewer_app $compiled_document`;
...
sub fail_unless_system {
	...
	system("echo"); # Obskurer bug: Ohne dieses echo bleibt die Ausgabe nach dem Tex compilieren stehen
	# und wird erst nach einem speichern weiter ausgeben
